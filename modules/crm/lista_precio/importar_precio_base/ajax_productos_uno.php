...
if (is_array($array_reg)) {
    $php_result = $clslistaprecio->editar_status_productos();
    foreach ($array_reg as $row) {
        if (!is_null($row[0])) {
            ...
            /////////////////////////////////////////////////////////////////////////////
            if (!$clslistaprecio->validar_existencias_producto_principal($codigo_producto)) {
                $id_producto = $clslistaprecio->get_id_producto($codigo_producto);
                $nombre_producto = $clslistaprecio->get_nombre_producto_por_cod($codigo_producto);
                if ($clslistaprecio->validar_existencias_productos($clslistaprecio->get_id_producto($codigo_producto))) {
                    if ($cantidad == 11) {
                        $codigo_producto = $row[0];
                        if ($php_result = $clslistaprecio->insert_precio_productos($fecha_subida, $id_producto, $codigo_producto, $nombre_producto, $precio)) {
                            $php_estado = true;
                        }
                    } else {
