...
$image = htmlspecialchars($_FILES['file_prod']['name']);
$ruta = htmlspecialchars($_FILES['file_prod']['tmp_name']);

$php_fileexten = strrchr($_FILES['file_prod']['name'],".");
$php_serial = strtoupper(substr(hash('sha1', $_FILES['file_prod']['name'].$php_fechatime),0,40)).$php_fileexten;
...
if($php_result= $cls_importdata->insert_prod($new_arrayf)){
    $php_estado = true;
}
