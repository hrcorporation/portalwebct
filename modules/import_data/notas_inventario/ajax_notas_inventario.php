...
if (is_array($array_reg)) {
    foreach ($array_reg as $row) {
        ...
        if(!is_null($row[0])){
            ...
            $new_array['valor_total'] = str_replace(",","",$row[6]);
            $new_array['valor_iva'] = str_replace(",","",$row[7]);
            $new_array['total_mas_valor_iva'] = str_replace(",","",$row[8]);
            $new_array['iva'] = str_replace(",","",$row[9]);
            $new_array['base_iva'] = str_replace(",","",$row[10]);
            $new_array['t_iva'] = $row[11];
            $new_array['ico'] = str_replace(",","",$row[12]);
