...
if (isset($_GET['txt_fecha_ini']) && isset($_GET['txt_fecha_fin'])) {
    ...
    $spreadsheet->setActiveSheetIndex(0)
        ...
        ->setCellValue('I1', 'HORA DE CARGUE')
        ->setCellValue('J1', 'HORA DE MIXER EN OBRA')
        ->setCellValue('K1', 'PLACA DE MIXER')
        ->setCellValue('L1', 'NOMBRE DEL CONDUCTOR')
        ->setCellValue('M1', 'REQUIERE BOMBA CONCRE TOLIMA')
        ->setCellValue('N1', 'TIPO DE DESCARGUE')
        ->setCellValue('O1', 'TIPO DE BOMBA')
        ->setCellValue('P1', 'METROS TUBERIA')
        ->setCellValue('Q1', 'FECHA INICIAL')
        ->setCellValue('R1', 'FECHA FINAL')
        ->setCellValue('S1', 'ELEMENTOS A FUNDIR')
        ->setCellValue('T1', 'OBSERVACIONES')
        ->setCellValue('U1', 'NOMBRE USUARIO CREACION')
        ->setCellValue('V1', 'FECHA CREACION');        
    $x = 2;
    ...
    if (is_array($datos)) {
        foreach ($datos as $fila) {
            ...
            $spreadsheet->setActiveSheetIndex(0)
                ...
                ->setCellValue('H' . $x, $fila['nombre_linea_produccion'])
                ->setCellValue('I' . $x, $fila['hora_cargue'])
                ->setCellValue('J' . $x, $fila['hora_mixer_obra'])
                ->setCellValue('K' . $x, $fila['mixer'])
                ->setCellValue('L' . $x, $fila['nombre_conductor'])
                ->setCellValue('M' . $x, $fila['requiere_bomba'])
                ->setCellValue('N' . $x, $fila['nombre_tipo_descargue'])
                ->setCellValue('O' . $x, $fila['tipo_bomba'])
                ->setCellValue('P' . $x, $fila['metros_tuberia'])
                ->setCellValue('Q' . $x, $fila['fecha_ini'])
                ->setCellValue('R' . $x, $fila['fecha_fin'])
                ->setCellValue('S' . $x, $fila['elementos_fundir'])
                ->setCellValue('T' . $x, $fila['observaciones'])
                ->setCellValue('U' . $x, $fila['nombre_usuario'])
                ->setCellValue('V' . $x, $fila['fecha_creacion']);
            $x++;
        }
    }
    // Rename worksheet
