...
if(isset($_POST['C_NumeroID'])&&isset($_POST['C_nombres']) &&isset($_POST['C_Usuario']) ){
    ...
    $TipoTercero = 3;
     ...
     $sql = "UPDATE `ct1_terceros` SET `ct1_Estado` = ? , ct1_TipoTercero = ? ,`ct1_NumeroIdentificacion`= ?, `ct1_RazonSocial` = ?, `ct1_Nombre1` = ?,  `ct1_Apellido1` = ?, `ct1_id_cliente1` = ?, `ct1_usuario` = ?, `ct1_pass` = ?, `ct1_rol` = ? WHERE ct1_IdTerceros = ? ";
        $stmt = mysqli_prepare($conexion_bd->myconn, $sql);
        $stmt->bind_param("iissssissii", $estado,$TipoTercero,$C_NumeroID,$razonSocial,$C_nombres,$C_Apellidos,$id_cliente1,$C_Usuario,$C_Pass,$rol,$id_user);
